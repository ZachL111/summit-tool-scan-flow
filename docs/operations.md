...
`summit-tool-scan-flow` is centered on cli tools work in Python. The technical target is: Package a Python local lab for scan analysis with negative fixtures, human-readable error snapshots, and documented operating limits.
...
The score is deterministic. The current threshold is 166. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 3.
